//
//  SettingsModalWindow.swift
//  JamAI
//
//  Settings modal window using the same pattern as TeamMemberModalWindow
//
...
final class SettingsScrollRoutingContentView: NSView {
...
class SettingsModalWindow: NSObject, NSWindowDelegate {
    ...
    func show() {
        ...
        let contentView = SettingsView(viewModel: viewModel, appState: appState, onDismiss: { [weak self] in
            self?.close()
        })
        ...
        let containerView = SettingsScrollRoutingContentView(frame: NSRect(x: 0, y: 0, width: panelWidth, height: panelHeight))
        ...
        // Show as modal sheet if possible
